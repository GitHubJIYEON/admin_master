...
import {
    INITIAL_SURVEY_TEMPLATES,
    type SurveyTemplateStatus,
    type SurveyTemplateVisibility,
...
function getInitialForm(template: (typeof INITIAL_SURVEY_TEMPLATES)[number] | undefined): FormState {
    ...
    return {
        ...
        questionCount: String(template.questionCount),
...
function SurveyTemplateEditPageInner() {
    const { templateId } = useParams();
    const navigate = useNavigate();
    const template = INITIAL_SURVEY_TEMPLATES.find((t) => t.id === templateId);

    const [form, setForm] = useState<FormState>(() => getInitialForm(template));
    ...
    return (
        <div className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-8">
            <div>
                <h1 className="text-2xl font-bold tracking-tight">템플릿 수정</h1>
                <p className="text-muted-foreground">설문 템플릿 정보를 수정합니다.</p>
            </div>
            ...
            <form className="rounded-lg border bg-card p-6" onSubmit={handleSubmit}>
                ...
                <div className="mt-6 flex items-center justify-end gap-2">
                    <Button asChild variant="outline" type="button">
                        <NavLink to={`/surveys/templates/${templateId}`}>취소</NavLink>
                    </Button>
...
export function SurveyTemplateEditPage() {
    const { templateId } = useParams();
    return <SurveyTemplateEditPageInner key={templateId} />;
}
