...
import {
    ...
    type PaymentMethod,
    type PaymentStatus,
} from "@/pages/sales-status-data";
import { PRODUCT_ROWS } from "@/pages/sales-products-data";
...
export function SalesStatusEditPage() {
    ...
    const [form, setForm] = React.useState<PaymentEditForm>(() => ({
        ...
        organizationName: "",
        organizationCode: "",
        productQuantities: sales
            ? Object.fromEntries(
                  sales.productNames
                      .map((name) => {
                          const product = PRODUCT_ROWS.find((p) => p.productName === name);
                          return product ? [product.id, 1] : null;
                      })
                      .filter(Boolean) as [string, number][],
              )
            : {},
        totalAmount: sales ? String(sales.finalPrice) : "",
        memo: "",
    }));
                ...
                <form className="rounded-lg border bg-card p-4 md:p-6" onSubmit={handleSubmit}>
                    <div className="grid gap-4 md:grid-cols-2">
                        ...
                        <div className="flex flex-col gap-4 rounded-lg border bg-muted/30 p-4 text-sm md:col-span-2">
                            <h3 className="font-semibold">금액 입력</h3>
                            <label className="flex flex-col gap-1.5">
                                <span className="text-muted-foreground">
                                    최종 금액 (VAT 포함) <span className="text-rose-500">*</span>
                                </span>
                                <div className="flex items-center gap-2">
                                    <Input
                                        type="number"
